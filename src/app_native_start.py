...
class App(app_cleanroom.App):
    """V154 Native Start: conexión read-only + único START normal 2/1."""
    ...
    def _install_fresh_core(self):
        ...
        if current is not None and current.vacuum is vacuum:
            return current
        self._fresh_core = NativeStartCore(vacuum, source="gui")
        self._v141_log(
            "V154 Native Start instalado · legado bloqueado antes de escribir"
        )
    ...
    def _on_connected(self, ip):
        ...
        try:
            ...
            self._set_banner(
                "V154 no pudo instalar el bloqueo read-only: "
        ...
        self._set_banner(
            "V154 Native Start · conectado en sólo lectura. "
    ...
    def _baseline_disabled(self, feature):
        messagebox.showinfo(
            ...
            "Primero vamos a validar un aspirado normal con un único START 2/1 "
    ...
    def start_clean(self):
        ...
        self._set_banner(
            "V154 · prueba limpia: lectura previa + UN START 2/1. "
    ...
    def _baseline_stop_worker(self, core):
        try:
            core.request_stop()
            self._post_ui("fresh_control_ok", "STOP enviado por V154.")
        except Exception as exc:
    ...
    def _baseline_dock_worker(self, core):
        try:
            ...
            self._post_ui(
                "fresh_control_ok",
                "Regreso a base solicitado por V154.",
            )
        except Exception as exc:
            self._post_ui("fresh_control_error", str(exc))
    ...
    def _v138_reset_cleaning_state(
        ...
        raise RuntimeError(
            "V154 bloqueó el reset/neutralización heredado."
        )
    ...
    def _v138_start_factory_edge(self, vacuum, label):
        return {
            ...
            "success": False,
            "movement_commands": 0,
            "error": "EDGE deshabilitado en Native Start",
        }
    ...
    def _handle_ui_event(self, kind, payload):
        if kind == "baseline_clean_started":
            ...
            self._set_banner(
                "V154 · aspirado normal activo · único START 2/1 enviado."
            )
            return None
        ...
        if kind == "baseline_clean_finished":
            ...
            if diag.get("error"):
                self._set_banner(
                    "V154 · prueba finalizó con error: "
                    + str(diag.get("error"))
                )
            else:
                self._set_banner(
                    "V154 · prueba física terminada · "
    ...
    def _diagnostic_text(self):
        ...
        lines = [
            "V154 NATIVE START BASELINE · ESTADO LIMPIO POST FACTORY RESET",
            "======================================================",
            ...
            "regla V154: conectar = sólo lectura",
            "regla V154: abrir la app = cero escrituras físicas",
            "regla V154: aspirado normal = lectura/precheck + UN A2/1",
            "regla V154: no escribe mode/sweep/repeat/suction/water antes del START",
            "regla V154: no usa 7/3, EDGE, build-map, zonas, punto, remoto ni twice-clean",
            "regla V154: mapeo, dirigidas, paredes, manual y Scheduler quedan bloqueados",
            "regla V154: STOP y Volver a base sólo salen por acción explícita del usuario",
            "",
            "AUDITORÍA V154:",
