...
class NativeStartCore:
    """V154: baseline físico mínimo después de factory reset.

    Conectar/leer nunca escribe. La única orden de inicio permitida es A2/1,
    ...
    def _run_normal(self, on_started, on_finished):
        ...
        try:
            ...
            # Misma frontera que cancelación: o sale A2/1 o gana STOP/DOCK.
            ...
            if not diag["started"] and not session.cancel_action:
                raise RuntimeError(
                    "El único START 2/1 no confirmó actividad física."
                )
